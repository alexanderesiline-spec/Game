...
import {Download, RefreshCw, Loader2, CheckCircle, XCircle, ChevronLeft, ChevronRight, Users} from "lucide-react";
import toast from "react-hot-toast";
import {comicsApi, type ComicJob, type ComicPages, type PanelData} from "@/lib/api";
          ...
          <div className="mb-10">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold text-gray-300">
                {job?.status === "failed" ? "Generation Failed" : "Generating your comic..."}
              </h2>
                ...
                {pages.pages.map((_, i) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition ${
                      i === currentPage ? "bg-purple-500 w-5" : "bg-[#2e2e4a]"
                    }`}
                  />
                ))}
              </div>
    ...
    >
      {panels.map((panel, i) => (
        <div
          ...
          style={{
            aspectRatio: style === "manhwa" ? "16/9" : panel.size === "splash" ? "4/3" : "3/4",
          }}
        >
          {panel.image?.url ? (
            <Image
